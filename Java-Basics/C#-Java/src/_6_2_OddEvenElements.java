...
public class _6_2_OddEvenElements {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < numbersArray.length; i++) {
			BigDecimal currentNumber = new BigDecimal(""
					+ Double.parseDouble(numbersArray[i]));
		...
		if (numbersArray.length > 1) {
			...
			System.out.printf("OddSum=%1$.1s, OddMin=%2$.1s, OddMax=%3$.1s"
					+ " EvenSum=No, EvenMin=No, EvenMax=No", oddSum, oddMin,
					oddMax);
